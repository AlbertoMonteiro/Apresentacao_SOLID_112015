...
{
    bool PodeCalcular(string medida) => medida == "unidade";

    decimal Calcular(ItemCarrinho item) => item.Quantidade * 5;
}

class CalculadorRegraPeso : ICalculadorRegra
{
    bool PodeCalcular(string medida) => medida == "peso";
...
{
    bool PodeCalcular(string medida) => medida == "especial";
